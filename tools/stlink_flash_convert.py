...
path = "G:/My Drive/Study/Thesis/Mar 2022/Datasets/3_a4"
path2 = "../data/nucleo"
file = open(path, "rb")
header = file.read(4)
...
while word:
    ...
    if last_seq_number > sequence_number:
        print(f"Last sequence {last_seq_number} number exceeds current {sequence_number}. Considering dropped.")
    elif sequence_number-last_seq_number > 1000:
        print(f"Last sequence {last_seq_number} number differentiated with more than 1000 compared to current {sequence_number}. Considering dropped.")  
    else:
        ...
        data_indices_time.append(index_to_time(sequence_number))
    # Iterate
    word = file.read(4)
    last_seq_number = sequence_number
    counter += 1
...
ax3 = axs[1]
l3 = ax3.plot(data_indices_time, PER_set, label='PER')
ax4 = ax3.twinx()
l4 = ax4.plot(data_indices_time, label='sequence_numbers')
ax3.legend(handles=l3+l4)
ax3.set_title("PER over time")

l1 = ax1.plot(data_indices_time, data_set_rrsi, color='orange', label='RSSI')
